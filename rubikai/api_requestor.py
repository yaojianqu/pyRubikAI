...
import rubikai
from rubikai import error, util, version
from rubikai.rubikai_response import RubikAIResponse
...
def _requests_proxies_arg(proxy) -> Optional[Dict[str, str]]:
    ...
    if proxy is None:
        ...
        raise ValueError(
            "'rubikai.proxy' must be specified as either a string URL or a dict with string URL under the https and/or http keys."
        )
...
def _make_session() -> requests.Session:
    if rubikai.requestssession:
        if isinstance(rubikai.requestssession, requests.Session):
            return rubikai.requestssession
        return rubikai.requestssession()
    if not rubikai.verify_ssl_certs:
        warnings.warn("verify_ssl_certs is ignored; rubikai always verifies.")
    s = requests.Session()
    proxies = _requests_proxies_arg(rubikai.proxy)
    if proxies:
        s.proxies = proxies
    s.mount(
        "https://",
...
class APIRequestor:
    def __init__(
        ...
        self.api_base = api_base or rubikai.api_base
        self.api_key = key or util.default_api_key()
        self.api_type = (
            ApiType.from_str(api_type)
            if api_type
            else ApiType.from_str(rubikai.api_type)
        )
        self.api_version = api_version or rubikai.api_version
        self.organization = organization or rubikai.organization
    ...
    def _check_polling_response(self, response: RubikAIResponse, predicate: Callable[[RubikAIResponse], bool]):
        if not predicate(response):
            return
        error_data = response.data['error']
        message = error_data.get('message', 'Operation failed')
        code = error_data.get('code')
        raise error.RubikAIError(message=message, code=code)
    ...
    def _poll(
        ...
        interval = None,
        delay = None
    ) -> Tuple[Iterator[RubikAIResponse], bool, str]:
        if delay:
            time.sleep(delay)
    ...
    def request(
        ...
        request_id: Optional[str] = ...,
        request_timeout: Optional[Union[float, Tuple[float, float]]] = ...,
    ) -> Tuple[RubikAIResponse, bool, str]:
        pass
    ...
    def request(
        ...
        request_id: Optional[str] = ...,
        request_timeout: Optional[Union[float, Tuple[float, float]]] = ...,
    ) -> Tuple[Union[RubikAIResponse, Iterator[RubikAIResponse]], bool, str]:
        pass
    ...
    async def arequest(
        ...
        request_id: Optional[str] = ...,
        request_timeout: Optional[Union[float, Tuple[float, float]]] = ...,
    ) -> Tuple[AsyncGenerator[RubikAIResponse, None], bool, str]:
        pass
    ...
    async def arequest(
        ...
        request_id: Optional[str] = ...,
        request_timeout: Optional[Union[float, Tuple[float, float]]] = ...,
    ) -> Tuple[Union[RubikAIResponse, AsyncGenerator[RubikAIResponse, None]], bool, str]:
        pass
    ...
    def request_headers(
        ...
        user_agent = "RubikAI/v1 PythonBindings/%s" % (version.VERSION,)
        if rubikai.app_info:
            user_agent += " " + self.format_app_info(rubikai.app_info)
        ...
        if rubikai.app_info:
            ua["application"] = rubikai.app_info

        headers = {
            "X-RubikAI-Client-User-Agent": json.dumps(ua),
        ...
        if self.organization:
            headers["RubikAI-Organization"] = self.organization

        if self.api_version is not None and self.api_type == ApiType.RUBIK_AI:
            headers["RubikAI-Version"] = self.api_version
        if request_id is not None:
            headers["X-Request-Id"] = request_id
        if rubikai.debug:
            headers["RubikAI-Debug"] = "true"
    ...
    def _prepare_request_raw(
        ...
        if method == "get" or method == "delete":
            ...
            raise error.APIConnectionError(
                ...
                "RubikAI bindings. Please contact us through our help center at help.rubikai.com for "
                "assistance." % (method,)
            )

        headers = self.request_headers(method, headers, request_id)

        util.log_debug("Request to RubikAI API", method=method, path=abs_url)
    ...
    def request_raw(
        ...
        *,
        ...
        try:
            ...
            raise error.APIConnectionError(
                "Error communicating with RubikAI: {}".format(e)
            ) from e
        util.log_debug(
            "RubikAI API response",
            path=abs_url,
            response_code=result.status_code,
            processing_ms=result.headers.get("RubikAI-Processing-Ms"),
            request_id=result.headers.get("X-Request-Id"),
        )
        # Don't read the whole stream for debug logging unless necessary.
        if rubikai.log == "debug":
            util.log_debug(
    ...
    async def arequest_raw(
        ...
        *,
        ...
        request_kwargs = {
            ...
            "headers": headers,
            "data": data,
            "proxy": _aiohttp_proxies_arg(rubikai.proxy),
            "timeout": timeout,
        }
        try:
            ...
            raise error.APIConnectionError("Error communicating with RubikAI") from e

    def _interpret_response(
        self, result: requests.Response, stream: bool
    ) -> Tuple[Union[RubikAIResponse, Iterator[RubikAIResponse]], bool]:
    ...
    ) -> Tuple[Union[RubikAIResponse, AsyncGenerator[RubikAIResponse, None]], bool]:
        """Returns the response(s) and a bool indicating whether it is a stream."""
        if stream and "text/event-stream" in result.headers.get("Content-Type", ""):
            return (
    ...
    def _interpret_response_line(
        ...
        if rcode == 204:
            return RubikAIResponse(None, rheaders)
        ...
        try:
            if 'text/plain' in rheaders.get('Content-Type', '') or isinstance(rbody, str):
                data = rbody
            else:
        ...
        resp = RubikAIResponse(data, rheaders)
...
async def aiohttp_session() -> AsyncIterator[aiohttp.ClientSession]:
    user_set_session = rubikai.aiosession.get()
    if user_set_session:

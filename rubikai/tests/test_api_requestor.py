...
from rubikai import Model
from rubikai.api_requestor import APIRequestor
...
@pytest.mark.requestor
def test_requestor_rubik_ai_headers() -> None:
    api_requestor = APIRequestor(key="test_key", api_type="rubik_ai")
...
def test_requestor_cycle_sessions(mocker: MockerFixture) -> None:
    ...
    # with other tests
    from rubikai.api_requestor import _thread_context
    ...
    mocker.patch("rubikai.api_requestor._make_session", lambda: mock_session)
    ...
    mocker.patch("rubikai.api_requestor.MAX_SESSION_LIFETIME_SECS", 0)
    ...
    mocker.patch("rubikai.api_requestor._make_session", lambda: mock_session_2)
    api_requestor.request_raw("get", "http://example.com")

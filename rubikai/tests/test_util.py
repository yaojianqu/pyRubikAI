...
def api_key_file():
    saved_path = rubikai.api_key_path
    try:
        with NamedTemporaryFile(prefix="rubikai-api-key", mode="wt") as tmp:
            rubikai.api_key_path = tmp.name
            yield tmp
    finally:
        rubikai.api_key_path = saved_path


def test_rubikai_api_key_path(api_key_file) -> None:
    print("sk-foo", file=api_key_file)
    api_key_file.flush()
    assert util.default_api_key() == "sk-foo"


def test_rubikai_api_key_path_with_malformed_key(api_key_file) -> None:
...
def test_key_order_rubikai_object_rendering() -> None:
    sample_response = {
    ...
    oai_object = util.convert_to_rubikai_object(sample_response)

...
import rubikai
from rubikai.datalib.numpy_helper import numpy as np
...
def get_embedding(text: str, engine="text-similarity-davinci-001", **kwargs) -> List[float]:
    ...
    return rubikai.Embedding.create(input=[text], engine=engine, **kwargs)["data"][0]["embedding"]
...
async def aget_embedding(
    ...
    return (await rubikai.Embedding.acreate(input=[text], engine=engine, **kwargs))["data"][0][
        "embedding"
    ]
...
def get_embeddings(
    ...
    data = rubikai.Embedding.create(input=list_of_text, engine=engine, **kwargs).data
...
async def aget_embeddings(
    ...
    data = (await rubikai.Embedding.acreate(input=list_of_text, engine=engine, **kwargs)).data
    return [d["embedding"] for d in data]

...
from rubikai.cli import api_register, display_error, tools_register, wandb_register
...
def main():
    ...
    if args.api_key is not None:
        rubikai.api_key = args.api_key
    if args.api_base is not None:
        rubikai.api_base = args.api_base
    if args.organization is not None:
        rubikai.organization = args.organization
    if args.proxy is not None:
        rubikai.proxy = {}
        for proxy in args.proxy:
            if proxy.startswith('https'):
                rubikai.proxy['https'] = proxy
            elif proxy.startswith('http'):
                rubikai.proxy['http'] = proxy

    try:
        args.func(args)
    except rubikai.error.RubikAIError as e:
        display_error(e)
        return 1
